...
class SceneStudyInputHandler:
    ...
    def handle_strongs_lookup(self):
        ...
        if sn_str:
            scene.showStrongsTooltip.emit(mouse_pos, None, None)
    ...
    def finish_arrow_drawing(self):
        ...
        self.clear_temp_arrow()

        if not end_key or end_key == start_key:
            scene._render_study_overlays(); scene.studyDataChanged.emit(); return
        ...
        act_s = menu.addAction("Standard")
        act_sn = menu.addAction("Snake")
        act_g = menu.addAction("Ghost")
